...
import { useForm, Controller, SubmitHandler, FieldValues, Control, ControllerRenderProps, Path } from "react-hook-form";

export const Form = <T extends Record<string, unknown>>({ children, onSubmit }: { children: React.ReactNode; onSubmit: (data: T) => void }) => {
    const methods = useForm<T>();
    return (
        <form onSubmit={methods.handleSubmit(onSubmit as SubmitHandler<FieldValues>)}>
            {React.Children.map(children, (child) =>
                React.isValidElement(child) ? React.cloneElement(child, { ...methods }) : child
            )}
        </form>
    );
};
    ...
    return (
        <Controller
            ...
            render={({ field }) => {
                ...
                throw new Error("The render function must return a valid ReactElement.");
            }}
        />
    );
};
